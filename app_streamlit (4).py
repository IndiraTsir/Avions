...
# BOXPLOT ==============================================================
    ...
    st.markdown('''
            ...
            - We can see that flight delays are not distributed equally
            - Majority of flights are expected to be early or on time
            - Even if most of the flights make good time, we still can see there seems to be a tendancy for some really extreme outliers.       
                    ''')  
...
# SEABORN PAS TOUCHE ====================================================
    ...
    st.markdown('''
            ## Observations
            - We can see that delays are distributed differently based on airlines
            - The months doesn't seem to have the same impact on all airlines
            - The destination of the airline compagny may be a good indicator for the delays (ex : Hawaiian Airlines in August)       
                    ''') 
...
# PAGE4 ==============================================================
    ...
    df_cat = df[['AIRLINE_NAME', 'ORIGIN_WAC', 'DEST', 'ARR_DELAY','MONTH']].copy(deep=True)
    df_cat['ARR_DELAY'] = df_cat['ARR_DELAY'] > 0
#     df_cat['MONTH'] = df_cat.FL_DATE.dt.month
    if cat == 'Airlines':
    # %%
        y =  df.groupby('AIRLINE_NAME')['ARR_DELAY'].mean()
        x = df.groupby('AIRLINE_NAME')['AIRLINE_NAME'].count()
        fig2 = px.scatter(data_frame=df, x=y.index, y=y.values, size=x, size_max=(60),
